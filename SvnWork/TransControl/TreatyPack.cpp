...
/***************************************************************
函 数 名：PackFileLen(DWORD _fLen)
功能描述：打包文件长度
参    数：DWORD _fLen: 文件总长度
			int _fnLen: 文件名的长度
全局变量：无
返 回 值: CTreatyPack& 本对象的引用
****************************************************************/
void CTreatyPack::PackFileLen(DWORD _fLen,int _fnLen)
{
	LONG64 dbTem = _fLen;
	LONG64 ab = 0;
	int a = 0;
	for (int i=0; i<6; i++)
	{
		...
		a = ab & 0x00ff;
		*(m_pDataBuf+i+109) = a;
		*(m_protocolPack.dataBuf+i+90) = a;
	}
	m_dataLength = 116;
	m_protocolPack.dataLen = 96;
	*(m_protocolPack.dataBuf+_fnLen) = 0;
	memcpy(m_pDataBuf+17,&m_protocolPack.dataLen,2);
	*(m_pDataBuf+19+_fnLen) = 0;
	byte byCalc = GetCheckCode();
	*(m_pDataBuf+115) = GetCheckCode();
